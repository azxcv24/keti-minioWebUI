...
const initWSConnection = (
  ...
  const port = isDev ? "9010" : url.port;

  // check if we are using base path, if not this always is `/`
  const baseLocation = new URL(document.baseURI);
  const baseUrl = baseLocation.pathname;

  const wsProt = isDev ? "wss" : wsProtocol(url.protocol);

  const wsurl = `${wsProt}://${url.hostname}:${port}${baseUrl}ws/objectManager`;
  console.log(wsurl);
  ...
  objectsWS.onerror = (err) => {
    wsInFlight = false;
    console.error("Error in websocket connection. Attempting reconnection...", err);
